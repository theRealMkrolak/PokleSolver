...
class Pokle: CustomStringConvertible {
    ...
    func getOptimalTable() async -> Table {
        ...
        for i in Progress(
            ...
            configuration: [
                ProgressPercent(), ProgressBarLine(barLength: 70), ProgressTimeEstimates(),
                ProgressStringWithUpdate { "Best Hand: \(optimalTable.board) \(maxSum)" },
            ])
        {
            let table = tables[i]
...
func parseDailyInput(filePath: String, index: Int) -> (Table, [(Card, Card)], Trophies) {
    do {
        ...
        // Return some default values or throw an error
        return (Table(board: .preFlop), [], Trophies(trophies: ((0, 1, 2), (0, 1, 2), (0, 1, 2))))
    }
}

func stupidFunction() -> Int {
...
let task = Task {
    ...
    if args.count > 1 && args[1] == "--input" && args.count > 2 {
        if let (parsedPlayers, parsedTrophies) = parseInputFile(filePath: args[2]) {
            ...
            let stateNames = ["Flop", "Turn", "River"]
            print(
                "\(stateNames[0]): \(trophies.trophies.0.0) \(trophies.trophies.0.1) \(trophies.trophies.0.2)"
            )
            print(
                "\(stateNames[1]): \(trophies.trophies.1.0) \(trophies.trophies.1.1) \(trophies.trophies.1.2)"
            )
            print(
                "\(stateNames[2]): \(trophies.trophies.2.0) \(trophies.trophies.2.1) \(trophies.trophies.2.2)"
            )
        }
    } else if args.count > 1 && args[1] == "--daily" && args.count > 2 {
        let (solution, hands, rankings) = parseDailyInput(
            filePath: args[2], index: (stupidFunction()))
        let pokle = Pokle(
            player1: hands[0], player2: hands[1], player3: hands[2], trophies: rankings)
